"""Deploy force software-update API + admin UI + OTA package (prebuilt; no npm on server)."""
import os
import re
import sys
import time
...
SRC = [
    ...
    "apps/server/src/remoteDeploy.ts",
    ...
    "apps/admin/src/styles/kiosks.css",
...
def main() -> None:
    ...
    n = put_tree(sftp, LOCAL / "apps/server/dist", f"{REMOTE}/apps/server/dist")
    print(f"server dist files: {n}")
    ...
    run(
        client,
        f"test -f {REMOTE}/apps/server/dist/softwareUpdatePending.js && "
        f"test -f {REMOTE}/apps/server/scripts/remote-force-update.ps1 && echo force-ota-files-ok",
    )
    print("DONE")
    client.close()
