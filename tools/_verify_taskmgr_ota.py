"""Force OTA itpc07 to 160614 and poll."""
...
def main():
    ...
    _, o, _ = c.exec_command(
        "grep -E '^(ADMIN_LOGIN|ADMIN_PASSWORD)=' /root/omskscrin/.env | sed 's/\\r//'")
    ...
    k = next(x for x in ks if "itpc07" in x["hostname"].lower())
    print("before", k.get("softwareVersion"))
    ...
    for i in range(45):
        try:
            ...
            h = {"error": str(ex)}
        print(f"+{i*3:02d}s ver={h.get('softwareVersion')} prog={h.get('updateInProgress')} err={h.get('error','')[:50]}")
